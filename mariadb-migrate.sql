-- MariaDB migration for cipicung-be.
-- Converted from full_backup.sql plus repository/migration fields used by the app.
-- Target: MariaDB 10.5+ / MySQL 8-compatible syntax.
...
DROP TABLE IF EXISTS potential_detail;
...
CREATE TABLE villages (
    ...
    created_at DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
    updated_at DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6),
    village_code VARCHAR(100) NULL,
    total_population INT NULL,
    total_family BIGINT NOT NULL DEFAULT 0,
    title VARCHAR(100) NULL,
    wide VARCHAR(100) NULL,
    mission JSON NULL,
    elevation VARCHAR(255) NULL,
    coordinate VARCHAR(255) NULL,
    population TEXT NULL,
    region TEXT NULL,
    hamlet_one BIGINT NULL,
    hamlet_two BIGINT NULL,
    total_rt BIGINT NOT NULL DEFAULT 0,
    total_rw BIGINT NOT NULL DEFAULT 0,
    rt_hamlet_one BIGINT NOT NULL DEFAULT 0,
    rt_hamlet_two BIGINT NOT NULL DEFAULT 0,
    rw_hamlet_one BIGINT NOT NULL DEFAULT 0,
    rw_hamlet_two BIGINT NOT NULL DEFAULT 0,
    north_border TEXT NULL,
    east_border TEXT NULL,
    south_border TEXT NULL,
    west_border TEXT NULL,
    area TEXT NULL,
    total_male BIGINT NOT NULL DEFAULT 0,
    total_female BIGINT NOT NULL DEFAULT 0,
    demographic_religions JSON NULL,
    demographic_religion_rt JSON NULL,
    demographic_education JSON NULL,
    demographic_occupation JSON NULL,
    demographic_ages JSON NULL,
    ig_usn VARCHAR(255) NULL,
    tiktok_usn VARCHAR(255) NULL,
    yt_usn VARCHAR(255) NULL,
    is_active TINYINT(1) NOT NULL DEFAULT 0,
    PRIMARY KEY (id),
    CHECK (mission IS NULL OR JSON_VALID(mission)),
    CHECK (demographic_religions IS NULL OR JSON_VALID(demographic_religions)),
    CHECK (demographic_religion_rt IS NULL OR JSON_VALID(demographic_religion_rt)),
    CHECK (demographic_education IS NULL OR JSON_VALID(demographic_education)),
    CHECK (demographic_occupation IS NULL OR JSON_VALID(demographic_occupation)),
    CHECK (demographic_ages IS NULL OR JSON_VALID(demographic_ages))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE potential_detail (
...
CREATE TABLE locations (
    ...
    updated_at DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6),
    PRIMARY KEY (id),
    CONSTRAINT fk_locations_created_by FOREIGN KEY (created_by_id) REFERENCES users(id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE businesses (
    ...
    KEY idx_business_category_id (category_id),
    KEY idx_business_location_id (location_id),
    KEY idx_business_media_id (media_id),
    CONSTRAINT businesses_category_id_fkey FOREIGN KEY (category_id) REFERENCES categories(id) ON DELETE SET NULL,
    CONSTRAINT businesses_location_id_fkey FOREIGN KEY (location_id) REFERENCES locations(id),
    CONSTRAINT businesses_media_id_fkey FOREIGN KEY (media_id) REFERENCES media(id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE documents (
    ...
    KEY idx_documents_media_id (media_id),
    CONSTRAINT documents_category_id_fkey FOREIGN KEY (category_id) REFERENCES categories(id) ON DELETE SET NULL,
    CONSTRAINT documents_uploaded_by_fkey FOREIGN KEY (uploaded_by) REFERENCES users(id) ON DELETE CASCADE,
    CONSTRAINT fk_document_media FOREIGN KEY (media_id) REFERENCES media(id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE events (
    ...
    KEY idx_events_author_id (author_id),
    ...
    CONSTRAINT fk_event_categories FOREIGN KEY (category_id) REFERENCES categories(id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE galleries (
    ...
    PRIMARY KEY (id),
    KEY idx_galleries_type (type),
    KEY idx_galleries_created_by (created_by),
    KEY idx_galleries_category_id (category_id),
    KEY idx_galleries_media_id (media_id),
    CONSTRAINT galleries_created_by_fkey FOREIGN KEY (created_by) REFERENCES users(id) ON DELETE CASCADE,
    CONSTRAINT galleries_category_id_fkey FOREIGN KEY (category_id) REFERENCES categories(id),
    CONSTRAINT fk_gallery_media FOREIGN KEY (media_id) REFERENCES media(id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE officials (
    ...
    PRIMARY KEY (id),
    KEY idx_official_order (order_number),
    KEY idx_officials_village_id (village_id),
    CONSTRAINT officials_village_id_fkey FOREIGN KEY (village_id) REFERENCES villages(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE posts (
    ...
    KEY idx_posts_publish_start (publish_start),
...
CREATE TABLE potentials (
    ...
    updated_at DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6),
    PRIMARY KEY (id),
    KEY idx_potentials_category_id (category_id),
    KEY idx_potentials_media_id (media_id),
    KEY idx_potentials_location_id (location_id),
    CONSTRAINT potentials_category_id_fkey FOREIGN KEY (category_id) REFERENCES categories(id) ON DELETE SET NULL,
    CONSTRAINT fk_potentials_media FOREIGN KEY (media_id) REFERENCES media(id),
    CONSTRAINT fk_potential_location FOREIGN KEY (location_id) REFERENCES locations(id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE user_session_log (
    ...
    PRIMARY KEY (id),
    KEY idx_user_session_log_user_id (user_id),
    CONSTRAINT user_session_log_user_id_fkey FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Data converted from data_postgres.sql.

-- Data for users
INSERT INTO users (id, role, password, is_active, last_login, created_at, updated_at, username, name) VALUES (6, 'admin', '$2a$10$dKemcSEvrqE4Q2k2Y7XKzeP1VqtnTW8amqfUAV.MvmN3Z.QRiEaSO', 1, '2026-07-17 21:23:08.349048', '2026-07-10 17:00:38.372401', '2026-07-17 21:23:08.349048', 'roby', 'ilham');
INSERT INTO users (id, role, password, is_active, last_login, created_at, updated_at, username, name) VALUES (5, 'admin', '$2a$10$vM7LYExPv2CNvn9kMilcC.0ov2UH3ZhXAQ/SPV790bg.aBMZY554a', 1, '2026-07-21 13:41:02.923259', '2026-07-04 10:57:40.252264', '2026-07-21 13:41:02.923259', 'ilhamgod14', 'ilham');
INSERT INTO users (id, role, password, is_active, last_login, created_at, updated_at, username, name) VALUES (12, 'admin', '$2a$10$5nUedfzzlzcTii7YxQOa1O8i6mHetbvwz1lD7KJDli7v90Bt/hvbO', 1, '2026-07-26 18:58:39.624646', '2026-07-26 16:13:15.508189', '2026-07-26 18:58:39.624646', 'ramadit', 'Ramadit');
INSERT INTO users (id, role, password, is_active, last_login, created_at, updated_at, username, name) VALUES (13, 'admin', '$2a$10$CMiRgT34Ud47bvVvOLIlW.9OsaE6h.3McoGlD8hH5yGCkXKJEPKum', 1, '2026-07-27 04:45:40.686745', '2026-07-27 04:45:15.075169', '2026-07-27 04:45:40.686745', 'cipicung26', 'Hermawan Sutisna');
INSERT INTO users (id, role, password, is_active, last_login, created_at, updated_at, username, name) VALUES (11, 'admin', '$2a$10$BifFVEBFCB.4Dm/VoLa9VOqQKExLJ91pZn1gYUrsVOYhRF/PIsX66', 1, '2026-07-29 02:32:18.87128', '2026-07-21 14:35:53.137457', '2026-07-29 02:32:18.87128', 'jia', 'Nabiilah Nur Fauziyyah');

-- Data for categories
INSERT INTO categories (id, name, slug, type, created_at, updated_at) VALUES (1, 'Dashboard', 'dashboard', 'dashboard', '2026-07-04 11:40:35.863008', '2026-07-04 11:40:35.863008');
INSERT INTO categories (id, name, slug, type, created_at, updated_at) VALUES (2, 'Pendidikan', 'pendidikan', 'news', '2026-07-09 00:21:26.60145', '2026-07-09 00:21:26.60145');
INSERT INTO categories (id, name, slug, type, created_at, updated_at) VALUES (3, 'Kuliner', 'kuliner', 'business', '2026-07-10 17:34:32.745891', '2026-07-10 17:34:32.745891');
INSERT INTO categories (id, name, slug, type, created_at, updated_at) VALUES (11, 'Galeri', 'galeri', 'gallery', '2026-07-20 16:32:39.906825', '2026-07-20 16:32:39.906825');
INSERT INTO categories (id, name, slug, type, created_at, updated_at) VALUES (12, 'Hasil Bumi', 'hasil-bumi', 'potential', '2026-07-25 07:34:12.220592', '2026-07-25 07:34:12.220592');
INSERT INTO categories (id, name, slug, type, created_at, updated_at) VALUES (13, 'Kerajinan', 'kerajinan', 'potential', '2026-07-25 07:34:22.154901', '2026-07-25 07:34:22.154901');
INSERT INTO categories (id, name, slug, type, created_at, updated_at) VALUES (14, 'Usaha Desa', 'usaha-desa', 'potential', '2026-07-25 07:34:40.676518', '2026-07-25 07:34:40.676518');
INSERT INTO categories (id, name, slug, type, created_at, updated_at) VALUES (15, 'Berita', 'berita', 'news', '2026-07-28 17:57:11.045616', '2026-07-28 17:57:11.045616');

-- Data for villages
INSERT INTO villages (id, name, province, regency, district, postal_code, address, phone, email, website, latitude, longitude, vision, history, description, created_at, updated_at, village_code, total_population, total_family, title, wide, mission, elevation, coordinate, population, region, hamlet_one, hamlet_two, north_border, east_border, south_border, west_border, area, is_active, total_rt, total_rw, rt_hamlet_one, rt_hamlet_two, rw_hamlet_one, rw_hamlet_two, ig_usn, tiktok_usn, yt_usn, total_male, total_female, demographic_religions, demographic_religion_rt, demographic_education, demographic_occupation, demographic_ages) VALUES (9, 'Kantor Kepala Desa Kantor Kepala Desa Kantor Kepala Desa Desa Cipicung', 'Jawa Barat', 'Purwakarta', 'Sukatani', '41167', '99QM+24C, Cipicung, Kec. Sukatani, Kabupaten Purwakarta, Jawa Barat 41167', '083124581734', 'desacipicung20@gmail.com', '', -6.6075, 107.37667, '"Terwujudnya Desa Cipicung yang lebih maju, berprestasi, berbudaya, dan kreatif melalui peningkatan sumber daya manusia, pengelolaan sumber daya alam, serta pemantapan pembangunan berlandaskan keagamaan, kultural, dan budaya daerah.”', 'Desa Cipicung pada awalnya merupakan bagian dari Desa Sukamaju. Seiring pemekaran wilayah, Cipicung berdiri sebagai desa tersendiri dengan Sukamaju sebagai desa induk.
...
INSERT INTO media (id, uploaded_by, entity_type, entity_id, role, original_name, file_name, file_path, mime_type, file_size, caption, order_number, created_at, updated_at) VALUES (34, 11, 'dashboard', 10, 'image', NULL, NULL, 'uploads/dashboard/1784907885094727750.jpg', 'image/jpeg', NULL, NULL, 0, '2026-07-24 15:44:45.102318', '2026-07-24 15:44:45.102318');
INSERT INTO media (id, uploaded_by, entity_type, entity_id, role, original_name, file_name, file_path, mime_type, file_size, caption, order_number, created_at, updated_at) VALUES (36, 11, 'village', 9, 'image', NULL, NULL, 'uploads/dashboard/1784951014718874154.jpg', 'image/jpeg', NULL, NULL, 0, '2026-07-25 03:43:34.719172', '2026-07-25 03:43:34.719172');
INSERT INTO media (id, uploaded_by, entity_type, entity_id, role, original_name, file_name, file_path, mime_type, file_size, caption, order_number, created_at, updated_at) VALUES (37, 11, 'gallery', 8, 'image', NULL, NULL, 'uploads/gallery/1784951922356789436.jpg', 'image/jpeg', NULL, NULL, 0, '2026-07-25 03:58:42.360174', '2026-07-25 03:58:42.360174');
INSERT INTO media (id, uploaded_by, entity_type, entity_id, role, original_name, file_name, file_path, mime_type, file_size, caption, order_number, created_at, updated_at) VALUES (38, 11, 'potential', 2, 'image', NULL, NULL, 'uploads/potentials/1784964960867553911.jpg', 'image/jpeg', NULL, NULL, 0, '2026-07-25 07:36:00.868339', '2026-07-25 07:36:00.868339');
INSERT INTO media (id, uploaded_by, entity_type, entity_id, role, original_name, file_name, file_path, mime_type, file_size, caption, order_number, created_at, updated_at) VALUES (39, 11, 'potential', 1, 'image', NULL, NULL, 'uploads/potentials/1784965149755233036.jpg', 'image/jpeg', NULL, NULL, 0, '2026-07-25 07:39:09.755517', '2026-07-25 07:39:09.755517');
INSERT INTO media (id, uploaded_by, entity_type, entity_id, role, original_name, file_name, file_path, mime_type, file_size, caption, order_number, created_at, updated_at) VALUES (40, 11, 'potential', 5, 'image', NULL, NULL, 'uploads/potentials/1784965236113994898.jpg', 'image/jpeg', NULL, NULL, 0, '2026-07-25 07:40:36.123517', '2026-07-25 07:40:36.123517');
INSERT INTO media (id, uploaded_by, entity_type, entity_id, role, original_name, file_name, file_path, mime_type, file_size, caption, order_number, created_at, updated_at) VALUES (41, 11, 'potential', 6, 'image', NULL, NULL, 'uploads/potentials/1784965347536814757.jpg', 'image/jpeg', NULL, NULL, 0, '2026-07-25 07:42:27.537131', '2026-07-25 07:42:27.537131');
INSERT INTO media (id, uploaded_by, entity_type, entity_id, role, original_name, file_name, file_path, mime_type, file_size, caption, order_number, created_at, updated_at) VALUES (42, 11, 'potential', 7, 'image', NULL, NULL, 'uploads/potentials/1784965484066473648.jpg', 'image/jpeg', NULL, NULL, 0, '2026-07-25 07:44:44.067309', '2026-07-25 07:44:44.067309');
...
INSERT INTO media (id, uploaded_by, entity_type, entity_id, role, original_name, file_name, file_path, mime_type, file_size, caption, order_number, created_at, updated_at) VALUES (44, 11, 'potential', 9, 'image', NULL, NULL, 'uploads/potentials/1784965545817117792.jpg', 'image/jpeg', NULL, NULL, 0, '2026-07-25 07:45:45.817327', '2026-07-25 07:45:45.817327');
INSERT INTO media (id, uploaded_by, entity_type, entity_id, role, original_name, file_name, file_path, mime_type, file_size, caption, order_number, created_at, updated_at) VALUES (48, 5, 'potential', 10, 'image', NULL, NULL, 'uploads/potentials/1784979307403777690.png', 'image/png', NULL, NULL, 0, '2026-07-25 11:35:07.40584', '2026-07-25 11:35:07.40584');
INSERT INTO media (id, uploaded_by, entity_type, entity_id, role, original_name, file_name, file_path, mime_type, file_size, caption, order_number, created_at, updated_at) VALUES (49, 5, 'gallery', 11, 'image', NULL, NULL, 'uploads/gallery/1784979321083141911.png', 'image/png', NULL, NULL, 0, '2026-07-25 11:35:21.088316', '2026-07-25 11:35:21.088316');
INSERT INTO media (id, uploaded_by, entity_type, entity_id, role, original_name, file_name, file_path, mime_type, file_size, caption, order_number, created_at, updated_at) VALUES (51, 5, 'news', 12, 'image', NULL, NULL, 'uploads/news/1784979342212410156.png', 'image/png', NULL, NULL, 0, '2026-07-25 11:35:42.214754', '2026-07-25 11:35:42.214754');
INSERT INTO media (id, uploaded_by, entity_type, entity_id, role, original_name, file_name, file_path, mime_type, file_size, caption, order_number, created_at, updated_at) VALUES (52, 11, 'gallery', 12, 'image', NULL, NULL, 'uploads/gallery/1785131077347336020.jpg', 'image/jpeg', NULL, NULL, 0, '2026-07-27 05:44:37.352084', '2026-07-27 05:44:37.352084');
INSERT INTO media (id, uploaded_by, entity_type, entity_id, role, original_name, file_name, file_path, mime_type, file_size, caption, order_number, created_at, updated_at) VALUES (54, 11, 'gallery', 13, 'image', NULL, NULL, 'uploads/gallery/1785254646144965910.jpg', 'image/jpeg', NULL, NULL, 0, '2026-07-28 16:04:06.155034', '2026-07-28 16:04:06.155034');
INSERT INTO media (id, uploaded_by, entity_type, entity_id, role, original_name, file_name, file_path, mime_type, file_size, caption, order_number, created_at, updated_at) VALUES (55, 11, 'gallery', 14, 'image', NULL, NULL, 'uploads/gallery/1785254732057109653.jpg', 'image/jpeg', NULL, NULL, 0, '2026-07-28 16:05:32.057771', '2026-07-28 16:05:32.057771');
...
INSERT INTO media (id, uploaded_by, entity_type, entity_id, role, original_name, file_name, file_path, mime_type, file_size, caption, order_number, created_at, updated_at) VALUES (57, 11, 'gallery', 15, 'image', NULL, NULL, 'uploads/gallery/1785267247721295928.jpg', 'image/jpeg', NULL, NULL, 0, '2026-07-28 19:34:07.722247', '2026-07-28 19:34:07.722247');
INSERT INTO media (id, uploaded_by, entity_type, entity_id, role, original_name, file_name, file_path, mime_type, file_size, caption, order_number, created_at, updated_at) VALUES (58, 11, 'gallery', 17, 'image', NULL, NULL, 'uploads/gallery/1785267334908715943.jpg', 'image/jpeg', NULL, NULL, 0, '2026-07-28 19:35:34.909576', '2026-07-28 19:35:34.909576');
INSERT INTO media (id, uploaded_by, entity_type, entity_id, role, original_name, file_name, file_path, mime_type, file_size, caption, order_number, created_at, updated_at) VALUES (59, 11, 'gallery', 18, 'image', NULL, NULL, 'uploads/gallery/1785267445195970659.jpg', 'image/jpeg', NULL, NULL, 0, '2026-07-28 19:37:25.196881', '2026-07-28 19:37:25.196881');
INSERT INTO media (id, uploaded_by, entity_type, entity_id, role, original_name, file_name, file_path, mime_type, file_size, caption, order_number, created_at, updated_at) VALUES (60, 11, 'gallery', 19, 'image', NULL, NULL, 'uploads/gallery/1785267555353481366.jpg', 'image/jpeg', NULL, NULL, 0, '2026-07-28 19:39:15.361742', '2026-07-28 19:39:15.361742');
INSERT INTO media (id, uploaded_by, entity_type, entity_id, role, original_name, file_name, file_path, mime_type, file_size, caption, order_number, created_at, updated_at) VALUES (61, 11, 'news', 13, 'image', NULL, NULL, 'uploads/news/1785268623194527847.jpg', 'image/jpeg', NULL, NULL, 0, '2026-07-28 19:57:03.196253', '2026-07-28 19:57:03.196253');
INSERT INTO media (id, uploaded_by, entity_type, entity_id, role, original_name, file_name, file_path, mime_type, file_size, caption, order_number, created_at, updated_at) VALUES (62, 11, 'news', 14, 'image', NULL, NULL, 'uploads/news/1785268690031812602.jpg', 'image/jpeg', NULL, NULL, 0, '2026-07-28 19:58:10.032708', '2026-07-28 19:58:10.032708');
INSERT INTO media (id, uploaded_by, entity_type, entity_id, role, original_name, file_name, file_path, mime_type, file_size, caption, order_number, created_at, updated_at) VALUES (63, 11, 'news', 15, 'image', NULL, NULL, 'uploads/news/1785268988016127391.jpg', 'image/jpeg', NULL, NULL, 0, '2026-07-28 20:03:08.016915', '2026-07-28 20:03:08.016915');
INSERT INTO media (id, uploaded_by, entity_type, entity_id, role, original_name, file_name, file_path, mime_type, file_size, caption, order_number, created_at, updated_at) VALUES (64, 11, 'news', 16, 'image', NULL, NULL, 'uploads/news/1785269107117288062.jpg', 'image/jpeg', NULL, NULL, 0, '2026-07-28 20:05:07.118158', '2026-07-28 20:05:07.118158');
INSERT INTO media (id, uploaded_by, entity_type, entity_id, role, original_name, file_name, file_path, mime_type, file_size, caption, order_number, created_at, updated_at) VALUES (65, 11, 'gallery', 21, 'image', NULL, NULL, 'uploads/gallery/1785292567671521393.jpg', 'image/jpeg', NULL, NULL, 0, '2026-07-29 02:36:07.675866', '2026-07-29 02:36:07.675866');
INSERT INTO media (id, uploaded_by, entity_type, entity_id, role, original_name, file_name, file_path, mime_type, file_size, caption, order_number, created_at, updated_at) VALUES (66, 11, 'gallery', 20, 'image', NULL, NULL, 'uploads/gallery/1785292752510843642.jpg', 'image/jpeg', NULL, NULL, 0, '2026-07-29 02:39:12.5197', '2026-07-29 02:39:12.5197');
INSERT INTO media (id, uploaded_by, entity_type, entity_id, role, original_name, file_name, file_path, mime_type, file_size, caption, order_number, created_at, updated_at) VALUES (67, 11, 'news', 19, 'image', NULL, NULL, 'uploads/news/1785292804665845783.jpg', 'image/jpeg', NULL, NULL, 0, '2026-07-29 02:40:04.667045', '2026-07-29 02:40:04.667045');
INSERT INTO media (id, uploaded_by, entity_type, entity_id, role, original_name, file_name, file_path, mime_type, file_size, caption, order_number, created_at, updated_at) VALUES (68, 11, 'news', 18, 'image', NULL, NULL, 'uploads/news/1785292831036796781.jpg', 'image/jpeg', NULL, NULL, 0, '2026-07-29 02:40:31.039413', '2026-07-29 02:40:31.039413');
INSERT INTO media (id, uploaded_by, entity_type, entity_id, role, original_name, file_name, file_path, mime_type, file_size, caption, order_number, created_at, updated_at) VALUES (69, 11, 'news', 17, 'image', NULL, NULL, 'uploads/news/1785292956551078572.jpg', 'image/jpeg', NULL, NULL, 0, '2026-07-29 02:42:36.552501', '2026-07-29 02:42:36.552501');

-- Data for businesses
INSERT INTO businesses (id, category_id, owner_name, business_name, description, phone, address, instagram, facebook, created_at, updated_at, location_id) VALUES (2, 1, 'ilham', 'aren gula', 'ilham', '0990099990', 'dskasdkdask', NULL, NULL, '2026-07-13 20:41:20.419611', '2026-07-13 20:41:20.419611', NULL);

-- Data for documents
INSERT INTO documents (id, category_id, uploaded_by, title, description, created_at, media_id, updated_at, source) VALUES (13, 15, 11, 'Dorong UMKM Go Digital, KKN UNSIKA 2026 Hadirkan Program CIKOBER di Desa Cipicung', 'Purwakarta, 11 Juli 2026 – Mahasiswa KKN UNSIKA 2026 Kelompok 23 melaksanakan program CIKOBER (Cipicung Ekonomi Berkembang) sebagai upaya mendukung transformasi digital UMKM di Desa Cipicung, Kecamatan Sukatani, Kabupaten Purwakarta. Kegiatan ini dilaksanakan dengan mengunjungi langsung warung dan pelaku UMKM yang ada di Desa Cipicung untuk memberikan pendampingan secara personal.
...
-- Data for galleries
INSERT INTO galleries (id, created_by, title, description, created_at, category_id, media_id, updated_at, is_active, type) VALUES (13, 5, 'Cek Kesehatan Gratis pada Masyarakat Desa Cipicung', 'Pemeriksaan kesehatan dan edukasi hidup sehat bagi masyarakat Desa Cipicung. Kolaborasi antara KKN UNSIKA 2026 dengan Puskesmas Sukatani', '2026-07-27 10:36:31.102654', 11, 54, '2026-07-29 12:19:08.57282', 0, 'gallery');
INSERT INTO galleries (id, created_by, title, description, created_at, category_id, media_id, updated_at, is_active, type) VALUES (14, 11, 'Sosialisasi dan Praktik BRIKUNG oleh KKN UNSIKA 2026', 'BRIKUNG (Briket Ramah Lingkungan Desa Cipicung) dengan memanfaatkan limbah sekam padi, diselenggarakan di Kantor Desa Cipicung', '2026-07-28 16:05:32.057771', 11, 55, '2026-07-29 12:19:08.57282', 0, 'gallery');
INSERT INTO galleries (id, created_by, title, description, created_at, category_id, media_id, updated_at, is_active, type) VALUES (15, 11, 'Senam Bersama Warga Desa Cipicung dan KKN UNSIKA 2026', 'Meningkatkan kesehatan, kebugaran, dan kebersamaan masyarakat Desa Cipicung melalui kegiatan senam bersama.', '2026-07-28 18:10:35.445796', 11, 57, '2026-07-29 12:19:08.57282', 0, 'gallery');
INSERT INTO galleries (id, created_by, title, description, created_at, category_id, media_id, updated_at, is_active, type) VALUES (18, 11, 'Tanaman Sayur (TAMSUR) untuk Ketahanan Pangan', 'Edukasi dan praktik budidaya tanaman sayur guna mendukung ketahanan pangan serta pemanfaatan lahan pekarangan masyarakat Desa Cipicung. Kolaborasi antara KKN UNSIKA 2026 dengan BPP Sukatani', '2026-07-28 19:37:25.196881', 11, 59, '2026-07-29 12:19:08.57282', 0, 'gallery');
INSERT INTO galleries (id, created_by, title, description, created_at, category_id, media_id, updated_at, is_active, type) VALUES (17, 11, 'Belajar Bersama di SDN Cipicung', 'Kegiatan pembelajaran interaktif untuk mendukung peningkatan semangat belajar dan wawasan siswa SDN Cipicung oleh KKN UNSIKA 2026', '2026-07-28 19:35:34.909576', 11, 58, '2026-07-29 12:19:08.57282', 0, 'gallery');
INSERT INTO galleries (id, created_by, title, description, created_at, category_id, media_id, updated_at, is_active, type) VALUES (16, 11, 'Ngosrek: Tradisi Gotong Royong Desa Cipicung', 'Tradisi kerja bakti masyarakat Desa Cipicung dalam menjaga kebersihan lingkungan serta mempererat kebersamaan warga.', '2026-07-28 18:14:41.136173', 11, 56, '2026-07-29 12:19:08.57282', 0, 'gallery');
INSERT INTO galleries (id, created_by, title, description, created_at, category_id, media_id, updated_at, is_active, type) VALUES (19, 11, 'Turnamen Sepak Bola Tingkat SD Antar-RT', 'Ajang kompetisi sepak bola bagi siswa tingkat SD antar-RT untuk menumbuhkan semangat sportivitas, kebersamaan, dan bakat olahraga di Desa Cipicung yang diselenggarakan oleh KKN UNSIKA 2026', '2026-07-28 19:39:15.361742', 11, 60, '2026-07-29 12:19:08.57282', 0, 'gallery');
INSERT INTO galleries (id, created_by, title, description, created_at, category_id, media_id, updated_at, is_active, type) VALUES (21, 11, 'CITRANG (Cipicung Terang)', 'Mahasiswa KKN UNSIKA melaksanakan pemasangan Penerangan Jalan Umum (PJU) berbasis panel surya untuk meningkatkan keamanan dan kenyamanan masyarakat Desa Cipicung.', '2026-07-29 02:26:59.767479', 11, 65, '2026-07-29 12:19:08.57282', 0, 'gallery');
INSERT INTO galleries (id, created_by, title, description, created_at, category_id, media_id, updated_at, is_active, type) VALUES (20, 11, 'Penyerahan Website SIPADES (Sistem Informasi Profil Desa)', 'Mahasiswa KKN UNSIKA 2026 menyerahkan website SIPADES kepada Pemerintah Desa Cipicung sebagai upaya mendukung digitalisasi informasi desa.', '2026-07-29 02:26:30.462189', 11, 66, '2026-07-29 12:19:08.57282', 0, 'gallery');
INSERT INTO galleries (id, created_by, title, description, created_at, category_id, media_id, updated_at, is_active, type) VALUES (8, 11, 'Sosialisasi dan Praktik BRIKUNG', 'BRIKUNG (Briket Ramah Lingkungan) dengan memanfaatkan limbah sekam padi oleh KKN UNSIKA 2026', '2026-07-23 04:35:28.189876', 1, 37, '2026-07-29 12:19:08.57282', 0, 'dashboard');
INSERT INTO galleries (id, created_by, title, description, created_at, category_id, media_id, updated_at, is_active, type) VALUES (10, 11, 'Cipicung', 'Terletak di kaki Gunung Kacapi, Desa Cipicung tumbuh bersama potensi alamnya. Aktivitas pertambangan batu belah, pertanian, dan usaha masyarakat menjadi denyut kehidupan yang menggerakkan desa setiap hari.', '2026-07-24 08:24:17.677424', 1, 34, '2026-07-29 12:19:08.57282', 1, 'dashboard');

-- Data for officials
INSERT INTO officials (id, village_id, name, `position`, phone, email, description, order_number, is_active, created_at, updated_at, start_date, finish_date) VALUES (26, 9, 'Joko Mulyono', 'sekretaris-desa', NULL, NULL, NULL, 2, 1, '2026-07-25 03:33:55.086297', '2026-07-25 03:48:23.847043', NULL, NULL);
INSERT INTO officials (id, village_id, name, `position`, phone, email, description, order_number, is_active, created_at, updated_at, start_date, finish_date) VALUES (27, 9, 'Sadari Wiharta', 'Kaur-Tata-Usaha-&-Umum', NULL, NULL, NULL, 3, 1, '2026-07-25 03:49:07.790755', '2026-07-25 04:01:14.36942', NULL, NULL);
INSERT INTO officials (id, village_id, name, `position`, phone, email, description, order_number, is_active, created_at, updated_at, start_date, finish_date) VALUES (28, 9, 'Cucu Intan Sari', 'Kaur-Keuangan', NULL, NULL, NULL, 4, 1, '2026-07-25 04:02:31.835011', '2026-07-25 04:02:31.835011', NULL, NULL);
INSERT INTO officials (id, village_id, name, `position`, phone, email, description, order_number, is_active, created_at, updated_at, start_date, finish_date) VALUES (29, 9, 'Muhamad Deni Jatnika', 'Kaur-Perencanaan', NULL, NULL, NULL, 5, 1, '2026-07-25 04:02:55.934047', '2026-07-25 04:02:55.934047', NULL, NULL);
INSERT INTO officials (id, village_id, name, `position`, phone, email, description, order_number, is_active, created_at, updated_at, start_date, finish_date) VALUES (30, 9, 'Abdul Jalal Wiharja', 'Kasi-Pemerintahan', NULL, NULL, NULL, 6, 1, '2026-07-25 04:03:16.6535', '2026-07-25 04:03:16.6535', NULL, NULL);
INSERT INTO officials (id, village_id, name, `position`, phone, email, description, order_number, is_active, created_at, updated_at, start_date, finish_date) VALUES (31, 9, 'Hermawan Sutisna', 'Kasi-Kesejahteraan', NULL, NULL, NULL, 7, 1, '2026-07-25 04:03:31.605127', '2026-07-25 04:03:31.605127', NULL, NULL);
INSERT INTO officials (id, village_id, name, `position`, phone, email, description, order_number, is_active, created_at, updated_at, start_date, finish_date) VALUES (32, 9, 'Ahmad Jaeni Tahir', 'Kasi-Pelayanan', NULL, NULL, NULL, 8, 1, '2026-07-25 04:03:49.487439', '2026-07-25 04:03:49.487439', NULL, NULL);
INSERT INTO officials (id, village_id, name, `position`, phone, email, description, order_number, is_active, created_at, updated_at, start_date, finish_date) VALUES (33, 9, 'Enjang', 'Kepala-Dusun-I', NULL, NULL, NULL, 9, 1, '2026-07-25 04:04:06.19182', '2026-07-25 04:04:06.19182', NULL, NULL);
INSERT INTO officials (id, village_id, name, `position`, phone, email, description, order_number, is_active, created_at, updated_at, start_date, finish_date) VALUES (34, 9, 'Asep Saepuloh', 'Kepala-Dusun-II', NULL, NULL, NULL, 10, 1, '2026-07-25 04:04:25.721888', '2026-07-25 04:04:25.721888', NULL, NULL);
INSERT INTO officials (id, village_id, name, `position`, phone, email, description, order_number, is_active, created_at, updated_at, start_date, finish_date) VALUES (65, 9, 'Lili Sadili', 'kepala-desa', '', '', '', 0, 1, '2026-07-27 03:26:05.244848', '2026-07-27 03:26:05.244848', '2013-01-01', '2026-07-24');
INSERT INTO officials (id, village_id, name, `position`, phone, email, description, order_number, is_active, created_at, updated_at, start_date, finish_date) VALUES (66, 9, 'Edi Supriadi', 'kepala-desa', '', '', '', 0, 0, '2026-07-27 03:26:05.244848', '2026-07-27 03:26:05.244848', '2007-01-01', '2012-12-31');
INSERT INTO officials (id, village_id, name, `position`, phone, email, description, order_number, is_active, created_at, updated_at, start_date, finish_date) VALUES (67, 9, 'Muhidin', 'kepala-desa', '', '', '', 0, 0, '2026-07-27 03:26:05.244848', '2026-07-27 03:26:05.244848', '2000-01-01', '2006-12-31');
INSERT INTO officials (id, village_id, name, `position`, phone, email, description, order_number, is_active, created_at, updated_at, start_date, finish_date) VALUES (68, 9, 'Asep Hidayat', 'kepala-desa', '', '', '', 0, 0, '2026-07-27 03:26:05.244848', '2026-07-27 03:26:05.244848', '1987-01-01', '1998-12-31');
INSERT INTO officials (id, village_id, name, `position`, phone, email, description, order_number, is_active, created_at, updated_at, start_date, finish_date) VALUES (69, 9, 'Mahpudin', 'kepala-desa', '', '', '', 0, 0, '2026-07-27 03:26:05.244848', '2026-07-27 03:26:05.244848', '1983-01-01', '1986-12-31');

-- Data for potentials
INSERT INTO potentials (id, title, subtitle, description, owner_name, owner_msisdn, category_id, slug, media_id, location_id, created_at, updated_at) VALUES (7, 'Gula Aren', 'RT 10', 'Nira disadap setiap pagi dan sore dari pohon aren di sekitar desa, lalu dimasak perlahan di atas tungku kayu hingga mengental dan dicetak menjadi gula batok. Proses ini dikerjakan turun-temurun oleh warga sebagai salah satu sumber penghasilan utama di lua', NULL, NULL, 12, 'gula-aren', 42, NULL, '2026-07-25 07:44:44.067309', '2026-07-25 07:44:44.067309');
